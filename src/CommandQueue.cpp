...
CommandQueue::CommandQueue() {
    // Create dummy node
    Node* dummy = new Node(Commands::Print{0, ""});
...
void CommandQueue::Push(Command&& cmd) {
    // Multi-producer (any lua thread)
    Node* new_node = new Node(std::move(cmd));
...
std::vector<Command> CommandQueue::PopAll() {
    ...
    while (next) {
        ...
        commands.push_back(std::move(next->data));
