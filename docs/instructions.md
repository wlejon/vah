...
currently when i drag a workflow node and drop it, for one or two frames the UI shows the original position before snapping to the new position. what i believe is happening is that we are persisting to the db, updating from bound information before it's updated, loading the newly saved db information. what i'd like is for the UI to show what the user is doing without updating from the bound data. this doesn't mean to creating a cache layer. this means to keep the bound data the same until the user updates it. the db/server lua will tell you when the user has updated it. 
